...
Create new database
************************************************************************************************/
CREATE DATABASE db_SQLCaseStudies_ADV;
USE db_SQLCaseStudies_ADV;
...
LEFT JOIN DIM_LOCATION AS L ON T.IDLocation = L.IDLocation
...
Manufacturer_Name, 
(TOTALPRICE/Quantity) AS PHONE_COST 
FROM FACT_TRANSACTIONS AS T
INNER JOIN DIM_MODEL AS M ON M.IDModel = T.IDModel
INNER JOIN DIM_MANUFACTURER AS MA ON MA.IDManufacturer = M.IDManufacturer
ORDER BY PHONE_COST;
...
INNER JOIN (SELECT *
			...
			WHERE DATEPART(YEAR,[DATE])=2009) AS T
...
((SELECT IDMODEL
FROM (SELECT *,
		ROW_NUMBER() OVER (ORDER BY SUM_QUANTITY DESC) AS RN
		FROM (SELECT IDMODEL, 
...
SELECT Manufacturer_Name
FROM (SELECT Manufacturer_Name
		FROM FACT_TRANSACTIONS AS A
		INNER JOIN DIM_MODEL AS B ON A.IDModel = B.IDModel
		INNER JOIN DIM_MANUFACTURER AS C ON C.IDManufacturer = B.IDManufacturer
		WHERE DATEPART(YEAR,[DATE]) = 2010) AS T1
...
FROM (SELECT Manufacturer_Name
		...
		INNER JOIN DIM_MODEL AS B ON A.IDModel = B.IDModel
		INNER JOIN DIM_MANUFACTURER AS C ON C.IDManufacturer = B.IDManufacturer
		WHERE DATEPART(YEAR,[DATE]) = 2009) AS T2
